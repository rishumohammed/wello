    ...
    <div class="metric-card-hero mb-6" id="home-hero-metric">
      <div class="flex items-center justify-between flex-wrap gap-2 mb-2">
        ...
        <div v-if="stats.target > 0" class="target-comparison-badge" :class="stats.isAboveTarget ? 'above' : 'below'" id="hero-target-comparison">
          ...
          <span v-else>{{ Math.abs(stats.targetDeltaPct) }}% Below Target</span>
        </div>
      </div>

      <!-- Main Hourly Rate -->
      <div class="hero-value-row">
        <div class="hero-unit">{{ store.currency }}</div>
      ...
      <div class="hero-target" v-if="stats.target > 0" id="hero-target-bar-wrap">
        ...
        <div class="hero-target-bar">
          <div class="hero-target-fill" :style="{ width: stats.progressPct + '%' }"></div>
        </div>
        <span class="hero-target-text fw-600">{{ Math.round(stats.progressPct) }}%</span>
      </div>
...
const maxChartRate = computed(() => {
  ...
  const maxVal = Math.max(...data.map(d => d.rate || 0), stats.value.target || 350)
