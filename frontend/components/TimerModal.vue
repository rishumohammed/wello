...
function handleStartTimer() {
  ...
  // Prevent duplicate active timers: handled safely by store.startTimer
  store.startTimer({
    projectId: form.projectId,
    title: form.title.trim(),
    type: form.type,
    paymentType: form.paymentType,
    unpaidReason: form.paymentType === 'intentional_unpaid' ? form.unpaidReason : null,
  })
...
function handleStopTimer() {
  ...
  if (result) {
    toast.success(`Work session saved (${store.minutesToHM(result.session?.durationMin || 1)}).`)
  }
  emit('stopped')
  emit('close')
}
