...
// Unified IANA Timezone, Boundary Math & DST-Safe Date Engine using Day.js
...
export function getUserToday(userTimezone: string = 'UTC'): string {
  const tz = isValidTimezone(userTimezone) ? userTimezone : 'UTC'
  return dayjs().tz(tz).format('YYYY-MM-DD')
}

export function getUserDayRange(dateStr?: string, userTimezone: string = 'UTC'): DateRangeUtc {
  const tz = isValidTimezone(userTimezone) ? userTimezone : 'UTC'
  const targetDate = dateStr || getUserToday(tz)

  const startLocal = dayjs.tz(targetDate, tz).startOf('day')
  const endLocal = dayjs.tz(targetDate, tz).endOf('day')
...
export function getUserWeekRange(dateStr?: string, userTimezone: string = 'UTC'): DateRangeUtc {
  const tz = isValidTimezone(userTimezone) ? userTimezone : 'UTC'
  const targetDate = dateStr || getUserToday(tz)
  const current = dayjs.tz(targetDate, tz)

  // Monday-based week start (ISO 8601 standard)
  const dayOfWeek = current.day() // 0 = Sun, 1 = Mon ... 6 = Sat
  const daysFromMonday = dayOfWeek === 0 ? 6 : dayOfWeek - 1

  const monday = current.subtract(daysFromMonday, 'day').startOf('day')
  const sunday = monday.add(6, 'day').endOf('day')

  const startUtc = monday.utc().toDate()
  const endUtc = sunday.utc().toDate()
...
export function getUserMonthRange(dateStr?: string, userTimezone: string = 'UTC'): DateRangeUtc {
  const tz = isValidTimezone(userTimezone) ? userTimezone : 'UTC'
  const targetDate = dateStr || getUserToday(tz)
  const current = dayjs.tz(targetDate, tz)
  ...
  const startUtc = firstDay.utc().toDate()
  const endUtc = lastDay.utc().toDate()
...
/**
 * Calculates absolute session duration in minutes based on UTC timestamps.
 * This is 100% immune to DST daylight saving shifts or clock adjustments.
 */
...
export const IANA_TIMEZONES = [
  ...
  // Americas
  { group: 'Americas', value: 'America/New_York', label: 'New York, Boston, Miami, Toronto, Montreal (EST/EDT, UTC-5/-4)' },
  ...
  { group: 'Americas', value: 'America/Los_Angeles', label: 'Los Angeles, San Francisco, Seattle, Vancouver (PST/PDT, UTC-8/-7)' },
